...
使用方法：
    ...
    python figureweave.py --method_file paper_method.txt --output_dir ./output --api_key "your-key"

    # 使用 OpenRouter
    python figureweave.py --method_file paper_method.txt --output_dir ./output --api_key "sk-or-v1-xxx" --provider openrouter

    # 使用 box 模式（传入坐标）
    python figureweave.py --method_file paper_method.txt --output_dir ./output --placeholder_mode box

    # 使用多个 SAM3 prompts 检测
    python figureweave.py --method_file paper_method.txt --output_dir ./output --sam_prompt "icon,diagram,arrow"

    # 跳过步骤 4.6 优化（设置迭代次数为 0）
    python figureweave.py --method_file paper_method.txt --output_dir ./output --optimize_iterations 0

    # 设置步骤 4.6 优化迭代 3 次
    python figureweave.py --method_file paper_method.txt --output_dir ./output --optimize_iterations 3

    # 自定义 box 合并阈值（0.8）
    python figureweave.py --method_file paper_method.txt --output_dir ./output --merge_threshold 0.8

    # 禁用 box 合并
    python figureweave.py --method_file paper_method.txt --output_dir ./output --merge_threshold 0
"""
...
GEMINI_IMAGE_RETRY_BASE_DELAY = float(
    os.environ.get("GEMINI_IMAGE_RETRY_BASE_DELAY", "15")
)
SVG_MAX_PLACEHOLDERS = max(1, int(os.environ.get("FIGUREWEAVE_MAX_PLACEHOLDERS", "10")))
SVG_MIN_BOX_AREA_RATIO = float(
    os.environ.get("FIGUREWEAVE_MIN_BOX_AREA_RATIO", "0.008")
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["openrouter", "bianxie", "gemini", "openai"],
        default=None,
        help="生图阶段 provider（可选，默认回退到 --provider）",
    )
    parser.add_argument("--image_api_key", default=None, help="生图阶段 API Key（可选，默认回退到 --api_key）")
    parser.add_argument("--image_base_url", default=None, help="生图阶段 API base URL（可选）")
    parser.add_argument(
        "--svg_provider",
        choices=["openrouter", "bianxie", "gemini", "openai", "anthropic"],
        default=None,
        help="SVG 理解和重建阶段 provider（可选，默认回退到 --provider）",
    )
    parser.add_argument("--svg_api_key", default=None, help="SVG 理解和重建阶段 API Key（可选，默认回退到 --api_key）")
    parser.add_argument("--svg_base_url", default=None, help="SVG 理解和重建阶段 API base URL（可选）")
    ...
    parser.add_argument("--figure_caption", default=None, help="Figure caption / figure brief（可选）")
    ...
    # 运行完整流程
    result = method_to_svg_candidates(
        method_text=method_text,
